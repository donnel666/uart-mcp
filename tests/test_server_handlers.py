...
class TestMainAndRunServer:
    ...
    async def test_run_server_starts_correctly(self):
        ...
        with patch("uart_mcp.server.mcp.server.stdio.stdio_server", return_value=mock_context):
            with patch("uart_mcp.server.server.run", new_callable=AsyncMock) as mock_run:
                await run_server()
    ...
    def test_main_normal_exit(self):
        ...
        with patch("uart_mcp.server.asyncio.run", side_effect=mock_asyncio_run_impl) as mock_asyncio_run:
